...
def get_option_chain_data(url, headers, cookies, symbol):
    no_error = True
    while(no_error):
        try:
            url = "https://www.nseindia.com/api/option-chain-indices?symbol=" + symbol
            bank_nifty_oi_data = requests.get(url, headers=headers, cookies=cookies)
            get_db(symbol, "indices_op_9").insert_one(bank_nifty_oi_data.json())
            print(symbol)
...
while(True):
    ...
    for index_symbol in indecis_symbol_list:
        get_option_chain_data(headers, cookies, index_symbol)
    # time.sleep(90)
